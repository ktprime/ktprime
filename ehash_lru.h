...
#define NEW_KVALUE(key, value, bucket) new(_pairs + bucket) PairT(key, value, bucket), _num_filled ++, _news ++
...
static uint32_t _news = 0;
static uint64_t _hits = 0, _finds = 0;
...
public:
    ...
    bool remove_half()
    {
        ...
        order_ids.reserve(_num_filled + 1);
        ...
        auto sorted_end = order_ids.begin() + order_ids.size() / 2;
        //TODO: BFPRT find the median element in O(n) or evaulate the medium by average
        std::nth_element(order_ids.begin(), sorted_end, order_ids.end());
        const auto medium_id = *sorted_end;
#else
        for (uint32_t src_bucket = 0; src_bucket < _num_buckets; src_bucket++)
            sumd += _pairs[src_bucket].orderid;
        const auto medium_id = uint32_t(sumd / _num_filled);
#endif

        const auto averge_id = uint32_t(sumd / _num_filled);
        const auto tnows = (uint32_t)time(0);
        uint32_t max_id = 1;

        //the core code. reset orderid is too large or small
        for (uint32_t src_bucket = 0; src_bucket < _num_buckets; src_bucket++) {
            auto& orderid = _pairs[src_bucket].orderid;
            if (orderid > medium_id) {
                if (orderid > max_id)
                    max_id = orderid;
#if EMHASH_LRU_TIME && EMHASH_TIME_DELAY > 100
                if (tnows < orderid - EMHASH_TIME_DELAY)
        ...
        char buff[256] = {0};
        snprintf(buff, sizeof(buff), "    _num_filled now-med max-med erase_ids =  %u %d %d %u | lf = %.3f hit = %.2lf%%, diff = %.4lf%% finds = %lu, news = %u, time_use = %d us",
                _num_filled, int(time(0) - medium_id), (max_id - medium_id), erase_ids - _num_filled,
                load_factor(), (100.0 * _hits) / _finds, (100.0*averge_id - 100.0*medium_id) / (1 + max_id - medium_id), _finds, _news, (int)(clock() - ts));
#if EMHASH_TAF_LOG
