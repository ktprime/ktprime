...
{
	assert(start % SEGMENT_SIZE == 0 && SEGMENT_SIZE % 19 == 0);
	const int sqrtn = (int)(sqrt((double)start + leng) + 0.1) + 1;
...
static int savePrime(const utype bitarray[], int leng, uchar prime[])
{
	...
	for (int p = 1; p < leng; p += 2) {
		if (!TST_BIT2(bitarray, p)) {
			prime[pn++] = p - lastpattern;
			lastpattern = p;
		}
	}

	prime[0] += diff;
	prime[pn] = leng - lastpattern;
...
static int initFirstPos(int s[], int startp, int p, int bitleng)
{
	...
	for (int i = 1; i < ns; i++) {
		int pdiff = KData.Kpattern[i] - lastp;
		lastp = KData.Kpattern[i];
		if (pdiff == 2) {
			si += mod2p;
		} else if (pdiff == 4) {
			si += mod4p;
...
static int sievePi1(utype bitarray[], const int pattern)
{
	...
	const int minp = sqrtn < bitleng ? sqrtn : bitleng;
	for (; p <= minp; PRIME_NEXT(p, k)) {
#if 1
	...
	for (; p <= sqrtn; PRIME_NEXT(p, k)) {
		const int s1 = asmMulDiv(Moudle[k], pattern, p);
		if (s1 < bitleng)
			SET_BIT(bitarray, s1);
	}
...
{
	const uint64 base = ipow(10, TABLE_GAP);
	const int wordleng = bitleng / 16;
	const int diff = pattern + Config.Kgap;
	int ktupels = 0;
...
static void printProgress(const int tid, const double tstart, int aves, int pcnt)
{
	...
	printf(", %s ~= %llu", KtupletName[Config.Ptk], curValue);
	if (lastValue > 0) {
		printf(", err ~= %.4lf%%%%",
...
static void printResult(const uint64 n, uint64 gptn, double ts)
{
	...
	if (Config.Ptk < PIKN) {
		...
		printf(PrintFormat[PIKN], KData.Kpattern[0] + 1, n, gptn);
	}
...
static uint64 Ktprime(const uint64 n, int pn)
{
	...
	if (n > atoint64("e13", 0)) {
		...
		KData.Wheel = wheel;
		if (CHECK_FLAG(PRINT_LOG)) {
			//			printf("PI%d[%d] = %llu, and time use %.lf ms\n",
			//					Config.Ptk, maxn, gptn, getTime() - ts);
		}
		addsmall = !addsmall;
	}

	if (Config.Ptk == PIKN && pn == 0 && n > ipow(10, TABLE_GAP)) {
...
static int startTest(int tesecase, bool rwflag)
{
	...
	for (int i = 1; i <= tesecase; i++) {
		if (!rwflag) {
			...
			if (gptn != res) {
				printf("case %d with wrong result %llu, ", i, gptn);
...
static void benchMark(const char cmdparams[][80])
{
	...
	for (int i = 0, ni = 1; cmdparams[i][0]; i++) {
		char c = cmdparams[i][0];
		if (isdigit(c) || toupper(c) == 'E') {
			uint64 ti = atoint64(cmdparams[i], 0);
			if (ni++ == 1) {
				start = ti;
			} else if (ni == 2) {
				n = ti;
...
static void testPik()
{
	...
	for (int i = 0; i < sizeof(pidata) / sizeof(pidata[0]); i++) {
		...
		for (int j = 1; j < sizeof(pidata[i])/sizeof(pidata[i][0]); j ++) {
			...
			uint r = atoint64(pidata[i][j] + 3, 0);
			uint t = Ktprime(n, 0);
			if (r != t) {
				printf("%u != %u fail\n", t, r);
			}
...
static void testPi2()
{
	...
	for (int i = 0; i < sizeof(pi2data) / sizeof(pi2data[0]); i++) {
		...
		for (int j = 1; j < 7; j ++) {
			...
			if (cal != ret) {
				printf("error: PI2_%d(%llu), cal = %llu, table ret = %llu\n",
...
static void printKpattern()
{
	...
	Config.Kgap = KData.Kpattern[ktuplets];
	if (ktuplets < PIKN) {
		...
		if (cmdc == 'H') {
			...
			for (int i = 0; cmdparams[cmdi + 2][i]; i++) {
				...
				if (Config.Ptk < 3 && Config.Ptk >= 0) {
					startTest(atoint64(cmdparams[cmdi + 1]), true);
				}
			}
		} else if (cmdc == 'L') {
