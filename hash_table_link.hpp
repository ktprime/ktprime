...
private:
    ...
    size_t erase_from_bucket(const KeyT& key) const
    {
        ...
        if (next_bucket == State::INACTIVE)
            ...
            std::swap(_pairs[next_bucket].second, _pairs[bucket].second);
    ...
    size_t find_filled_bucket(const KeyT& key) const
    {
        ...
        else if (next_bucket == State::INACTIVE)
            return (size_t)-1;
#elif 1
        if (next_bucket == State::INACTIVE)
            return (size_t)-1;
    ...
    size_t find_or_allocate(const KeyT& key)
    {
...
#if 0
        next_bucket = find_key_bucket(next_bucket, key);
        ...
        STAT_POS(_pairs, next_bucket) = new_bucket;
        return new_bucket;
    }
