...
static const uint MAX_BUCKET = (1 << 14) / 3; //> 10*2^32 / 2^18 * 30 = 10^14 / 3
static const uint WHEEL_SIZE = 1 << 11; //12: 32k, best in [10 - 13]
static const uint MEM_BLOCK = (1 << 24) / WHEEL_SIZE; //32 MB
...
static _Bucket Bucket [MAX_BUCKET + 4];
...
static uint ilog2(uint n, uint base)
{
	int powbase = 0;
	while (n / base) {
		powbase ++;
...
#define CHECK_OR(pbit, orbit)  if (pbit <= pend) *pbit |= orbit
static void crossOffWheelFactor(uchar* ppbeg[], const uchar* pend, const uint p)
{
	...
	CHECK_OR(ps0, BIT0); CHECK_OR(ps1, BIT1); CHECK_OR(ps2, BIT2);
	...
	CHECK_OR(ps0, BIT4); CHECK_OR(ps1, BIT5); CHECK_OR(ps2, BIT6);
}
...
static int initBucketInfo(const uint sieve_size, const uint sqrtp, uint64 range)
{
	...
	BucketInfo.CurBucket = range / sieve_size + 1;//overflow for large range
	...
	BucketInfo.Log2Size = ilog2(sieve_size / WHEEL30, 2);
	BucketInfo.SieveSize = (1 << BucketInfo.Log2Size) - 1;

	const uint pix = (uint)(sqrtp / log((double)sqrtp) * (1 + 1.200 / log((double)sqrtp)));

	//203280221 >> 12
	int blocks = pix / WHEEL_SIZE + MIN(BucketInfo.MaxBucket, BucketInfo.CurBucket) * 2 / 3;
	if (range < sqrtp / 4)
		blocks >>= 2;
...
{
	const uint maxp = Threshold.L1Maxp;
...
{
	const uint sqrtp = (uint)sqrt((double)start + sieve_size) + 0;
	const uint l1maxp = Threshold.L1Maxp;
	if (sqrtp < l1maxp)
		return ;

	uint j = Threshold.L1Index, p = l1maxp;
...
static void eratSieveMedium(uchar bitarray[], const uint64 start, uint sieve_size, const uint pindex, uint maxp)
{
	...
	for (uint wp1 = pwheel->Wp; wp1 < pmin; wp1 = pwheel->Wp) {
		const uint wi = wp1 >> SIBIT;
		const uchar pi = wp1 % (1 << SIBIT);
		const uint p = wi * WHEEL30 + Pattern30[pi];
...
#else
			uint wheel = sieve_index % (1 << SIBIT);
//			sieve_index >>= SIBIT;
...
static int segmentedSieve(const uint64 start, const uint sieve_size, const uint wheel_offset, Cmd* cmd = NULL)
{
	...
	for (uint sieve_index = 0, segsize = Threshold.L2Size; sieve_index < sieve_size; sieve_index += segsize) {
		...
		//sieve p in [L1_DCACHE_SIZE / 30, L2_DCACHE_SIZE / 30] 147/1720 180/1560
...
//		time_use += getTime() - ts; if (sieve_size != Config.SieveSize) { printf("eratSieveMedium1 time %.f ms\n", time_use); time_use = 0; }
	...
	//2.sieve p in [L2_DCACHE_SIZE / 30, sieve_size / 2] 654/1720 560/1560
...
//	time_use1 += getTime() - ts; if (sieve_size != Config.SieveSize) { printf("eratSieveMedium time %.f ms\n", time_use1); time_use1 = 0; }
	...
	if (BucketInfo.UseSieve) {
		...
		//3.sieve p [sieve_size / 2, sqrtp] 610/1720, 610/1560
...
static int setSieveSize(uint sieve_size)
{
	...
	if (sieve_size > MAX_CACHE * WHEEL30)
		sieve_size = MAX_CACHE * WHEEL30 - 8 * Threshold.L1Size;

	sieve_size = (1 << ilog2(sieve_size / WHEEL30 + 1, 2)) * WHEEL30;
...
{
	double ts = getTime();
	const uint sqrtp = (uint)sqrt((double)end) + 0;
...
//	assert (start <= end && (end - start) < 1e16);
	...
	uint64 primes = checkSmall(start, end, cmd);
	if (sieve_size < Threshold.L2Size && sqrtp >= Config.MinPrime && sqrtp > 10000000) {
	...
	initMediumWheel(sieve_size, medium + 256, segment_low, sieve_size < Threshold.L2Size);

	BucketInfo.UseSieve = sqrtp > minp && sqrtp >= Threshold.L2Maxp;
	if (BucketInfo.UseSieve) {
		...
		if (Config.Flag & PRINT_LOG) {
			...
			printf("	MaxIndex = %d, MaxBucket = %d\n\tStock remain = %2.2f%%, Mem = %d MB\n",
					BucketInfo.CurBucket, BucketInfo.MaxBucket, BucketInfo.CurStock * 100.0 / BucketInfo.StockSize,
	...
	if (BucketInfo.UseSieve) {
		for (int i = 0; i < BucketInfo.PtrSize; i++)
			free(WheelPtr[i]);
		assert(BucketInfo.StockSize == BucketInfo.CurStock && BucketInfo.CurBucket == 0);
...
static void printInfo( )
{
	...
	printf("[MARCO] : MAX_SIEVE = %dk, WHEEL_SIZE = %dk, ERATBIG = %d, WHEEL = %d\n",
...
//	uint64 start = ipow(10, 16); start -= start % WHEEL210;
//	doSievePrime(start, start + Config.SieveSize - 1);
//	doSievePrime(start, start + Config.SieveSize + 1);
...
PI[1e13, 1e13+1e10] = 334067230        7.17 /10.50
PI[1e14, 1e14+1e10] = 310208140        9.07 /12.81
PI[1e15, 1e15+1e10] = 289531946        11.06/14.94
PI[1e16, 1e16+1e10] = 271425366        13.03/17.10
PI[1e17, 1e17+1e10] = 255481287        15.40/19.78
PI[1e18, 1e18+1e10] = 241272176        19.05/24.27
PI[1e19, 1e19+1e10] = 228568014        25.34/33.16

PI[1e18, 1e18+1e6] = 24280              1.16/0.74
PI[1e18, 1e18+1e8] = 2414886            1.80/1.72
PI[1e18, 1e18+1e9] = 24217085           3.91/4.00
