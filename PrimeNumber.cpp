...
void check(stype a[], int n, int k)
{
	if (a[2*n] < 0)
		return;
...
int main(int argc, char* argv[])
{
	...
	if (argc > 1 && isdigit(argv[1][0])) { k = atoi(argv[1]); }
	if (argc > 2) {
		int ration = atoi(argv[2]);
		if (ration >= -100 && ration < 0) n = MAXN / (-ration);
		if (ration <= 100 && ration > 0) n = MAXN * ration;
		else if (ration > 0) n = ration;
	}

	stype* arr = (stype *)malloc(sizeof(stype) * (n * 2 + k + 8)) + 4;
	stype* buff = arr + n;
	arr[2 * n] = -1;
